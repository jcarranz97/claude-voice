...
def fresh(home, monkeypatch):
    ...
    hudcore._gpu_cache.update(t=0.0, val=None, card=None, name=None)
    hudcore._stat_cache.update(t=0.0, val={}, cpu=None)
...
def pci_ids(monkeypatch, home):
    ...
    monkeypatch.setattr(hudcore, "PCI_IDS", (str(home / "absent.ids"), str(db)))
    return db
...
class TestPciName:
    ...
    def test_the_subsystem_line_names_the_actual_board(self, pci_ids):
        assert hudcore._pci_name("1002:744c", "1da2", "5320") == "PULSE RX 7900 XTX"
    ...
    def test_an_unknown_subsystem_falls_back_to_the_bracketed_model(self, pci_ids):
        ...
        assert hudcore._pci_name("1002:744c", "0000", "0000") == "Radeon RX 7900 XT/7900 XTX/7900M"

    def test_a_device_with_no_bracket_is_used_whole(self, pci_ids):
        assert hudcore._pci_name("1002:1234", "0", "0") == "Plain Device Name"

    def test_a_vendor_that_is_not_listed_keeps_its_numbers(self, pci_ids):
        assert hudcore._pci_name("9999:0001", "0", "0") == "9999:0001"

    def test_something_that_is_not_a_device_id_is_returned_as_it_came(self, pci_ids):
        assert hudcore._pci_name("744c", "0", "0") == "744c"

    def test_no_database_anywhere_keeps_the_numbers(self, monkeypatch, home):
        monkeypatch.setattr(hudcore, "PCI_IDS", (str(home / "nope.ids"),))
        assert hudcore._pci_name("1002:744c", "0", "0") == "1002:744c"
...
class TestGpuCard:
    ...
    def test_the_card_with_the_most_memory_wins(self, monkeypatch, home):
        ...
        monkeypatch.setattr(hudcore, "DRM", drm)
        assert hudcore._gpu_card() == big

    def test_a_card_with_no_vram_file_is_not_a_candidate(self, monkeypatch, home):
        drm = home / "drm"
        a_card(drm, "card0")
        monkeypatch.setattr(hudcore, "DRM", drm)
        assert hudcore._gpu_card() is None

    def test_no_drm_directory_is_no_card(self, monkeypatch, home):
        monkeypatch.setattr(hudcore, "DRM", home / "nothing-here")
        assert hudcore._gpu_card() is None
    ...
    def test_a_drm_directory_that_will_not_list_is_no_card(self, monkeypatch):
        ...
        monkeypatch.setattr(hudcore, "DRM", types.SimpleNamespace(glob=_boom))
        assert hudcore._gpu_card() is None
...
class TestGpuSysfs:
    ...
    def test_a_card_that_does_not_say_what_it_is_is_just_a_gpu(self, monkeypatch, home):
        ...
        hudcore._gpu_cache["card"] = dev
        assert hudcore._gpu_sysfs()["name"] == "GPU"
    ...
    def test_the_name_is_looked_up_once(self, monkeypatch, home):
        ...
        hudcore._gpu_cache["card"] = dev
        calls = []
        monkeypatch.setattr(hudcore, "_pci_name", lambda p, sv, sd: calls.append(p) or "A Card")
        hudcore._gpu_sysfs()
        hudcore._gpu_sysfs()
...
class TestGpuNvidia:
    ...
    def test_without_the_tool_there_is_no_answer(self, monkeypatch):
        monkeypatch.setattr(hudcore.shutil, "which", lambda n: None)
        assert hudcore._gpu_nvidia() is None

    def test_the_csv_line_becomes_bytes_and_percentages(self, monkeypatch):
        monkeypatch.setattr(hudcore.shutil, "which", lambda n: "/usr/bin/nvidia-smi")
        monkeypatch.setattr(
            hudcore.subprocess,
            "run",
            lambda cmd, **kw: types.SimpleNamespace(stdout="GeForce RTX 4090, 31, 2048, 24564\n"),
        )
        g = hudcore._gpu_nvidia()
    ...
    def test_a_tool_that_answers_nonsense_reports_nothing(self, monkeypatch):
        monkeypatch.setattr(hudcore.shutil, "which", lambda n: "/usr/bin/nvidia-smi")
        monkeypatch.setattr(
            hudcore.subprocess, "run", lambda cmd, **kw: types.SimpleNamespace(stdout="")
        )
        assert hudcore._gpu_nvidia() is None
...
class TestGpuStats:
    ...
    def test_sysfs_is_preferred_and_the_percentage_is_added(self, monkeypatch):
        ...
        monkeypatch.setattr(hudcore, "_gpu_nvidia", lambda: pytest.fail("asked anyway"))
        assert hudcore.gpu_stats()["vram"] == 25.0

    def test_nvidia_is_the_fallback(self, monkeypatch):
        monkeypatch.setattr(hudcore, "_gpu_sysfs", lambda: None)
        monkeypatch.setattr(
            hudcore,
            "_gpu_nvidia",
        ...
        assert "vram" not in hudcore.gpu_stats()

    def test_a_machine_with_no_readable_card_shows_no_rows(self, monkeypatch):
        monkeypatch.setattr(hudcore, "_gpu_sysfs", lambda: None)
        monkeypatch.setattr(hudcore, "_gpu_nvidia", lambda: None)
        assert hudcore.gpu_stats() is None

    def test_a_reader_that_raises_shows_no_rows(self, monkeypatch):
        def _boom():
            raise OSError("sysfs")

        monkeypatch.setattr(hudcore, "_gpu_sysfs", _boom)
        assert hudcore.gpu_stats() is None

    def test_the_answer_is_held_for_a_second_and_a_half(self, monkeypatch):
        calls = []
        monkeypatch.setattr(hudcore, "_gpu_sysfs", lambda: calls.append(1) or None)
        monkeypatch.setattr(hudcore, "_gpu_nvidia", lambda: None)
        hudcore.gpu_stats()
        hudcore.gpu_stats()
        assert len(calls) == 1
...
class TestCpuSample:
    ...
    def test_it_reads_busy_and_total(self):
        busy, total = hudcore._cpu_sample()
        assert 0 < busy < total

    def test_off_linux_there_is_no_sample(self, monkeypatch):
        monkeypatch.setattr(hudcore, "Path", blocking_path("/proc/stat"))
        assert hudcore._cpu_sample() is None
...
class TestSystemStats:
    ...
    def test_the_first_reading_is_zero_because_nothing_was_measured_yet(self):
        assert hudcore.system_stats()["cpu"] == 0.0

    def test_the_second_reading_is_the_difference_between_samples(self, monkeypatch):
        hudcore._stat_cache["cpu"] = (100, 200)
        monkeypatch.setattr(hudcore, "_cpu_sample", lambda: (150, 400))
        assert hudcore.system_stats()["cpu"] == 25.0

    def test_a_counter_that_went_backwards_is_reported_as_idle(self, monkeypatch):
        hudcore._stat_cache["cpu"] = (100, 400)
        monkeypatch.setattr(hudcore, "_cpu_sample", lambda: (150, 200))
        assert hudcore.system_stats()["cpu"] == 0.0

    def test_the_answer_is_held_for_a_second_and_a_half(self, monkeypatch):
        first = hudcore.system_stats()
        monkeypatch.setattr(hudcore, "_cpu_sample", lambda: pytest.fail("resampled"))
        assert hudcore.system_stats() is first

    def test_unreadable_memory_is_zero_rather_than_an_empty_window(self, monkeypatch):
        monkeypatch.setattr(hudcore, "Path", blocking_path("/proc/meminfo"))
        s = hudcore.system_stats()
    ...
    def test_a_disk_that_will_not_answer_is_zero(self, monkeypatch):
        ...
        monkeypatch.setattr(hudcore.os, "statvfs", _boom)
        assert hudcore.system_stats()["disk"] == 0.0

    def test_a_platform_with_no_load_average_reports_none(self, monkeypatch):
        monkeypatch.delattr(hudcore.os, "getloadavg")
        assert hudcore.system_stats()["load"] == [0, 0, 0]
